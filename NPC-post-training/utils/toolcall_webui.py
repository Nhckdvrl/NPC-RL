...
st.set_page_config(page_title="多模型工具调用演示", layout="wide")
...
# 支持多个端口
PORTS = os.getenv("PORTS", "8112,8114").split(",")
...
from agent_r1.src.reward_score.model_evaluator import ModelEvaluator
...
class ToolManager(Singleton):
    def __init__(self):
        # 默认使用mss-nomem工具集
        self.tool_set = "mss-nomem"
        self.tools = _default_tools(self.tool_set)
        print(f"已加载工具: {', '.join([tool.name for tool in self.tools])}")
        ...
        for tool in self.tools:
            ...
            self.tool_functions[tool.name] = tool.execute
            
    def update_tools(self, tool_set):
        """更新工具集"""
        if tool_set != self.tool_set:
            self.tool_set = tool_set
            self.tools = _default_tools(tool_set)
            ...
            print(f"已更新工具集为 {tool_set}: {', '.join([tool.name for tool in self.tools])}")
...
class EvaluatorManager(Singleton):
    ...
    def evaluate_response(self, conversations, question, golden_answers=None, is_need_tool=True):
        # 获取最后一个助手回复
        ...
        try:
            # 格式化对话
            formatted_prompt = tokenizer.apply_chat_template(conversations, add_generation_prompt=False, tokenize=False)
            
            # 评估
            scores = self.model_evaluator.evaluate_response(
                response_str=formatted_prompt,
                question=question,
                is_need_tool=is_need_tool,
                ground_truth=golden_answers if golden_answers else [""],
...
with st.sidebar:
    ...
    # 工具集选择
    st.subheader("工具集选择")
    tool_options = ["math", "mss", "mss-nomem", "search", "wiki", "python", "calculator"]
    selected_tool_set = st.selectbox(
        "选择要使用的工具集",
        options=tool_options,
        index=tool_options.index("mss-nomem") if "mss-nomem" in tool_options else 0
    )
    
    # 更新工具集
    tool_manager = ToolManager.get_instance()
    tool_manager.update_tools(selected_tool_set)
    ...
    if query:
        with st.spinner("正在处理查询..."):
            try:
                ...
                # 处理查询并获取对话
                new_conversations = process_query(current_client.client, query, tools=tool_manager.openai_tools, tool_functions=tool_manager.tool_functions, model_name=model_name)
                ...
                if "tool_calls" in msg and msg["tool_calls"]:
                    with st.expander("工具调用"):
                        for tool_call in msg["tool_calls"]:
                            ...
                            st.markdown(f"**工具**: {function.get('name', '')}")
                            st.code(function.get('arguments', '{}'), language="json")
                st.markdown("---")
            elif role == "tool":
                st.markdown(f"**工具结果** ({msg.get('name', '')})")
        ...
        if eval_results:
            if 'error' in eval_results:
                ...
                for key, value in eval_results.items():
                    if key in ['relevance', 'coherence', 'tool_reasonability', 'groundedness'] and isinstance(value, dict):
...
st.markdown("---")
st.markdown("工具调用演示系统 - 基于VLLM API")
st.markdown("支持工具: " + ", ".join([tool.name for tool in tool_manager.tools]))
